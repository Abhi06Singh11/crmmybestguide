                ...
                {currentStep === 1 && (
                  <div className="space-y-4 animate-in fade-in-0">
                    <FormField control={form.control} name="projectName" render={({ field }) => (
                        <FormItem><FormLabel>Project Name</FormLabel><FormControl><Input placeholder="e.g., API Upgrade" {...field} /></FormControl><FormMessage /></FormItem>
                    )}/>
                ...
                {currentStep === 2 && (
                  <div className="space-y-4 animate-in fade-in-0">
                    <FormField control={form.control} name="developer" render={({ field }) => (
                        <FormItem><FormLabel>Assign Developer</FormLabel>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Select a developer" /></SelectTrigger></FormControl>
                            <SelectContent>
                                <SelectItem value="Bob Williams">Bob Williams</SelectItem>
                                <SelectItem value="Dana Scully">Dana Scully</SelectItem>
                                <SelectItem value="Grace Slick">Grace Slick</SelectItem>
                            </SelectContent>
                        </Select><FormMessage /></FormItem>
                    )}/>
                     <div className="grid md:grid-cols-2 gap-4">
                        <FormField control={form.control} name="budget" render={({ field }) => (
                            <FormItem><FormLabel>Budget ($)</FormLabel><FormControl><Input type="number" placeholder="e.g., 5000" {...field} /></FormControl><FormMessage /></FormItem>
                        )}/>
                    ...
                    <div className="space-y-4 p-4 border rounded-lg bg-muted/50 animate-in fade-in-0">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            <div><p className="text-muted-foreground">Client</p><p className="font-medium">{form.getValues('clientName') || 'Not set'}</p></div>
                             <div><p className="text-muted-foreground">Developer</p><p className="font-medium">{form.getValues('developer') || 'Not set'}</p></div>
       ...
       <Dialog open={isSuccessModalOpen} onOpenChange={setIsSuccessModalOpen}>
        <DialogContent className="sm:max-w-md">
            ...
            <div className="flex justify-center gap-2 pt-4">
                ...
                <Link href="/d/admin/developer/projects">
